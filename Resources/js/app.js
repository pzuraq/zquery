...
app.controller('ZqueryController', ['$scope', '$filter', 'queryService', 'ngTableParams', function($scope, $filter, queryService, ngTableParams) {
  ...
  $scope.submit = function() {
    ...
    if (searchParams.beds && searchParams.beds.length > 0) {
      params.push(searchParams.beds);
    }
    ...
    if (searchParams.collate && !searchParams.zip) {
      ...
      query = 'SELECT zip FROM zips WHERE state="' + state + '"';

      if (city.length > 0) query += 'AND (UPPER(primary_city)="' + city + '" OR UPPER(acceptable_cities) LIKE "%' + city + '%" OR UPPER(county)="' + city + '")'

      zips = db.execute(query);
